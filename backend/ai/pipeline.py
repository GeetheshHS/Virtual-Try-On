...
from .utils_catvton import (compute_vae_encodings, numpy_to_pil, prepare_image,
...
class CatVTONPipeline:
    def __init__(
        ...
        self.noise_scheduler = DDIMScheduler.from_pretrained(base_ckpt, subfolder="scheduler", local_files_only=local_only)
        ...
        if vae_ckpt and os.path.exists(vae_ckpt):
            ...
            paths_to_check = [
                os.path.join(os.path.dirname(base_ckpt), "sd-vae-ft-mse"),
                "backend/models/sd-vae-ft-mse",
        ...
        vae_local_only = os.path.exists(vae_path)
        self.vae = AutoencoderKL.from_pretrained(vae_path, local_files_only=vae_local_only).to(device, dtype=torch.float32)
        
        if not skip_safety_check:
            self.feature_extractor = CLIPImageProcessor.from_pretrained(base_ckpt, subfolder="feature_extractor", local_files_only=local_only)
            self.safety_checker = StableDiffusionSafetyChecker.from_pretrained(base_ckpt, subfolder="safety_checker", local_files_only=local_only).to(device, dtype=weight_dtype)
        else:
            self.feature_extractor = None
            self.safety_checker = None
            
        self.unet = UNet2DConditionModel.from_pretrained(base_ckpt, subfolder="unet", local_files_only=local_only).to(device, dtype=weight_dtype)
        ...
        # Enable TF32 for faster training/inference on Ampere/Ada Lovelace GPUs
        if use_tf32:
    ...
    def check_inputs(self, image, condition_image, mask, width, height):
        ...
        condition_image = condition_image.resize((width, height), PIL.Image.LANCZOS)
        return image, condition_image, mask
    ...
    def __call__(
        ...
        **kwargs
        ...
        concat_dim = -2  # y axis concat (height-wise) as per model checkpoint training layout
        ...
        mask_latent = torch.nn.functional.interpolate(mask, size=masked_latent.shape[-2:], mode="nearest").to(dtype=self.weight_dtype)
        ...
        # We wrap in a simple print/tqdm loop or pass callbacks
        ...
        image = self.vae.decode(latents.to(self.device, dtype=torch.float32)).sample
